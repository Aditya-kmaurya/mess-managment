...
const name = process.env.MUNSHI_NAME;
const email = (process.env.MUNSHI_EMAIL ).toLowerCase().trim();
const password = process.env.MUNSHI_PASSWORD ;
const hostel = (process.env.MUNSHI_HOSTEL ).toUpperCase().trim();
...
async function run() {
  try {
    ...
    if (existing && !shouldUpdate) {
      console.log('\n Munshi already exists with this email:');
